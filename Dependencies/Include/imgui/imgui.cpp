#pragma warning(disable : 6386 26495)
...
>>>>>>> refs/heads/master
 ...
 - 2017/12/21 (1.53) - ImDrawList: removed 'bool anti_aliased = true' parameter of ImDrawList::AddPolyline() and ImDrawList::AddConvexPolyFilled(). Prefer manipulating ImDrawList::Flags if you need to toggle them during the frame.
 ...
 - 2017/08/15 (1.51) - changed parameter order for BeginPopupContextWindow() from (char const*,int buttons,bool also_over_items) to (char const*,int buttons,bool also_over_items). Note that most calls relied on default parameters completely.
 ...
 - 2017/08/08 (1.51) - removed ColorEditMode() and ImGuiColorEditMode in favor of ImGuiColorEditFlags and parameters to the various Color*() functions. The SetColorEditOptions() allows to initialize default but the user can still change them with right-click context menu.
                     - changed prototype of 'ColorEdit4(char const* label, float col[4], bool show_alpha = true)' to 'ColorEdit4(char const* label, float col[4], ImGuiColorEditFlags flags = 0)', where passing flags = 0x01 is a safe no-op (hello dodgy backward compatibility!). - check and run the demo window, under "Color/Picker Widgets", to understand the various new options.
 ...
 - 2016/11/06 (1.50) - BeginChild(char const*) now applies the stack id to the provided label, consistently with other functions as it should always have been. It shouldn't affect you unless (extremely unlikely) you were appending multiple times to a same child from different locations of the stack id. If that's the case, generate an id with GetId() and use it instead of passing string to BeginChild().
...
#ifdef _MSC_VER
#pragma warning (disable: 26451 28182 26495 6011)
...
static float const NAV_WINDOWING_HIGHLIGHT_DELAY            = 0.20f;    // Time before the highlight and screen dimming starts fading in
static float const NAV_WINDOWING_LIST_APPEAR_DELAY          = 0.15f;    // Time before the window list starts to appear

// Window resizing from edges (when io.ConfigWindowsResizeFromEdges = true and ImGuiBackendFlags_HasMouseCursors is set in io.BackendFlags by back-end)
static float const WINDOWS_RESIZE_FROM_EDGES_HALF_THICKNESS = 4.0f;     // Extend outside and inside windows. Affect FindHoveredWindow().
static float const WINDOWS_RESIZE_FROM_EDGES_FEEDBACK_TIMER = 0.04f;    // Reduce visual noise by only highlighting the border after a certain time.
static float const WINDOWS_MOUSE_WHEEL_SCROLL_LOCK_TIMER    = 2.00f;    // Lock scrolled window (so it doesn't pick child windows that are scrolling through) for a certaint time, unless mouse moved.

// Docking
static float const DOCKING_TRANSPARENT_PAYLOAD_ALPHA        = 0.50f;    // For use with io.ConfigDockingTransparentPayload. Apply to Viewport _or_ WindowBg in host viewport.
...
// Settings
static void*            WindowSettingsHandler_ReadOpen(ImGuiContext*, ImGuiSettingsHandler*, char const* name);
static void             WindowSettingsHandler_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, char const* line);
...
static void             RenderWindowTitleBarContents(ImGuiWindow* window, const ImRect& title_bar_rect, char const* name, bool* p_open);
...
void ImTriangleBarycentricCoords(const ImVec2& a, const ImVec2& b, const ImVec2& c, const ImVec2& p, float& out_u, float& out_v, float& out_w)
{
    ...
    ImVec2 v1 = c - a;
    ImVec2 v2 = p - a;
    float const denom = v0.x * v1.y - v1.x * v0.y;
...
char* ImStrdupcpy(char* dst, size_t* p_dst_size, char const* src)
{
...
{
    char const* p = (char const*)memchr(str, (int)c, str_end - str);
    return p;
}
...
char const* ImStreolRange(char const* str, char const* str_end)
{
    char const* p = (char const*)memchr(str, '\n', str_end - str);
    return p ? p : str_end;
}
...
char const* ImStristr(char const* haystack, char const* haystack_end, char const* needle, char const* needle_end)
{
    ...
    char const un0 = (char)toupper(*needle);
...
char const* ImStrSkipBlank(char const* str)
{
    while (str[0] == ' ' || str[0] == '\t')
        str++;
    return str;
...
// FIXME-OPT: Replace with e.g. FNV1a hash? CRC32 pretty much randomly access 1KB. Need to do proper measurements.
ImU32 ImHashStr(char const* data_p, size_t data_size, ImU32 seed)
{
    seed = ~seed;
    ImU32 crc = seed;
    const unsigned char* data = (const unsigned char*)data_p;
...
ImFileHandle ImFileOpen(char const* filename, char const* mode)
{
#if defined(_WIN32) && !defined(IMGUI_DISABLE_WIN32_FUNCTIONS) && !defined(__CYGWIN__) && !defined(__GNUC__)
    // We need a fopen() wrapper because MSVC/Windows fopen doesn't handle UTF-8 filenames. 
    int const filename_wsize = ImTextCountCharsFromUtf8(filename, NULL) + 1;
    int const mode_wsize = ImTextCountCharsFromUtf8(mode, NULL) + 1;
...
void*   ImFileLoadToMemory(char const* filename, char const* mode, size_t* out_file_size, int padding_bytes)
{
...
int ImTextCharFromUtf8(unsigned int* out_char, char const* in_text, char const* in_text_end)
{
    ...
    if ((*str & 0xe0) == 0xc0)
    {
        *out_char = IM_UNICODE_CODEPOINT_INVALID; // will be invalid but not end of string
        if (in_text_end && in_text_end - (char const*)str < 2) return 1;
    ...
    {
        *out_char = IM_UNICODE_CODEPOINT_INVALID; // will be invalid but not end of string
        if (in_text_end && in_text_end - (char const*)str < 3) return 1;
    ...
    {
        *out_char = IM_UNICODE_CODEPOINT_INVALID; // will be invalid but not end of string
        if (in_text_end && in_text_end - (char const*)str < 4) return 1;
...
int ImTextStrFromUtf8(ImWchar* buf, int buf_size, char const* in_text, char const* in_text_end, char const** in_text_remaining)
{
    ImWchar* buf_out = buf;
...
int ImTextCountUtf8BytesFromChar(char const* in_text, char const* in_text_end)
{
...
bool ImGuiTextFilter::Draw(char const* label, float width)
{
    if (width != 0.0f)
...
void ImGuiTextBuffer::append(char const* str, char const* str_end)
{
    int len = str_end ? (int)(str_end - str) : (int)strlen(str);

    // Add zero-terminator the first time
    int const write_off = (Buf.Size != 0) ? Buf.Size : 1;
    int const needed_sz = write_off + len;
...
void ImGui::RenderTextWrapped(ImVec2 pos, char const* text, char const* text_end, float wrap_width)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
void ImGui::RenderTextClippedEx(ImDrawList* draw_list, const ImVec2& pos_min, const ImVec2& pos_max, char const* text, char const* text_display_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect)
{
...
void ImGui::RenderTextClipped(const ImVec2& pos_min, const ImVec2& pos_max, char const* text, char const* text_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect)
{
    ...
    int const text_len = (int)(text_display_end - text);
    if (text_len == 0)
        return;
...
void ImGui::RenderTextEllipsis(ImDrawList* draw_list, const ImVec2& pos_min, const ImVec2& pos_max, float clip_max_x, float ellipsis_max_x, char const* text, char const* text_end_full, const ImVec2* text_size_if_known)
{
    ImGuiContext& g = *GImGui;
    ...
    if (text_size.x > pos_max.x - pos_min.x)
    {
        ...
        float const font_size = draw_list->_Data->FontSize;
        char const* text_end_ellipsis = NULL;
        ...
        if (ellipsis_char_count > 1)
        {
            ...
            float const spacing_between_dots = 1.0f * (draw_list->_Data->FontSize / font->FontSize);
            ellipsis_glyph_width = glyph->X1 - glyph->X0 + spacing_between_dots;
            ellipsis_total_width = ellipsis_glyph_width * (float)ellipsis_char_count - spacing_between_dots;
        }

        // We can now claim the space between pos_max.x and ellipsis_max.x
        float const text_avail_width = ImMax((ImMax(pos_max.x, ellipsis_max_x) - ellipsis_total_width) - pos_min.x, 1.0f);
...
void ImGui::RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border, float rounding)
{
    ...
    float const border_size = g.Style.FrameBorderSize;
...
{
    float const h = draw_list->_Data->FontSize * 1.00f;
...
void ImGui::RenderNavHighlight(const ImRect& bb, ImGuiID id, ImGuiNavHighlightFlags flags)
{
    ...
    if (flags & ImGuiNavHighlightFlags_TypeDefault)
    {
        ...
        float const DISTANCE = 3.0f + THICKNESS * 0.5f;
...
{
    ImGuiID seed = IDStack.back();
    int const r_rel[4] = { (int)(r_abs.Min.x - Pos.x), (int)(r_abs.Min.y - Pos.y), (int)(r_abs.Max.x - Pos.x), (int)(r_abs.Max.y - Pos.y) };
...
void ImGui::ItemSize(const ImVec2& size, float text_baseline_y)
{
    ...
    float const offset_to_match_baseline_y = (text_baseline_y >= 0) ? ImMax(0.0f, window->DC.CurrLineTextBaseOffset - text_baseline_y) : 0.0f;
    float const line_height = ImMax(window->DC.CurrLineSize.y, size.y + offset_to_match_baseline_y);
...
bool ImGui::ItemAdd(const ImRect& bb, ImGuiID id, const ImRect* nav_bb_arg)
{
    ...
    // Clipping test
    bool const is_clipped = IsClippedEx(bb, id, false);
    if (is_clipped)
        return false;
...
bool ImGui::FocusableItemRegister(ImGuiWindow* window, ImGuiID id)
{
    ...
    // Increment counters
    bool const is_tab_stop = (window->DC.ItemFlags & (ImGuiItemFlags_NoTabStop | ImGuiItemFlags_Disabled)) == 0;
...
bool ImGui::DebugCheckVersionAndDataLayout(char const* version, size_t sz_io, size_t sz_style, size_t sz_vec2, size_t sz_vec4, size_t sz_vert, size_t sz_idx)
{
    bool error = false;
...
static ImDrawList* GetViewportDrawList(ImGuiViewportP* viewport, size_t drawlist_no, char const* drawlist_name)
{
...
void ImGui::StartMouseMovingWindowOrNode(ImGuiWindow* window, ImGuiDockNode* node, bool undock_floating_node)
{
    ...
    bool const clicked = IsMouseClicked(0);
    bool const dragging = IsMouseDragging(0, g.IO.MouseDragThreshold * 1.70f);
...
void ImGui::UpdateMouseWheel()
{
    ...
    if (g.IO.MouseWheel != 0.0f && g.IO.KeyCtrl && g.IO.FontAllowUserScaling)
    {
        ...
        float const new_font_scale = ImClamp(window->FontWindowScale + g.IO.MouseWheel * 0.10f, 0.50f, 2.50f);
        float const scale = new_font_scale / window->FontWindowScale;
    ...
    float const wheel_y = (g.IO.MouseWheel != 0.0f && !g.IO.KeyShift) ? g.IO.MouseWheel : 0.0f;
    ...
    float const wheel_x = (g.IO.MouseWheelH != 0.0f && !g.IO.KeyShift) ? g.IO.MouseWheelH : (g.IO.MouseWheel != 0.0f && g.IO.KeyShift) ? g.IO.MouseWheel : 0.0f;
...
void ImGui::UpdateHoveredWindowAndCaptureFlags()
{
    ...
    bool const mouse_avail_to_imgui = (mouse_earliest_button_down == -1) || g.IO.MouseDownOwned[mouse_earliest_button_down];

    // If mouse was first clicked outside of ImGui bounds we also cancel out hovering.
    // FIXME: For patterns of drag and drop across OS windows, we may need to rework/remove this test (first committed 311c0ca9 on 2015/02)
    bool const mouse_dragging_extern_payload = g.DragDropActive && (g.DragDropSourceFlags & ImGuiDragDropFlags_SourceExtern) != 0;
...
void ImGui::NewFrame()
{
    ...
    float const memory_compact_start_time = (g.IO.ConfigWindowsMemoryCompactTimer >= 0.0f) ? (float)g.Time - g.IO.ConfigWindowsMemoryCompactTimer : FLT_MAX;
...
static void ImGui::EndFrameDrawDimmedBackgrounds()
{
    ...
    ImGuiWindow* modal_window = GetTopMostPopupModal();
    bool const dim_bg_for_modal = (modal_window != NULL);
    bool const dim_bg_for_window_list = (g.NavWindowingTargetAnim != NULL);
...
ImVec2 ImGui::CalcTextSize(char const* text, char const* text_end, bool hide_text_after_double_hash, float wrap_width)
{
    ImGuiContext& g = *GImGui;
    ...
    ImFont* font = g.Font;
    float const font_size = g.FontSize;
    if (text == text_display_end)
...
int ImGui::CalcTypematicRepeatAmount(float t0, float t1, float repeat_delay, float repeat_rate)
{
    ...
    int const count_t0 = (t0 < repeat_delay) ? -1 : (int)((t0 - repeat_delay) / repeat_rate);
    int const count_t1 = (t1 < repeat_delay) ? -1 : (int)((t1 - repeat_delay) / repeat_rate);
    int const count = count_t1 - count_t0;
    return count;
}
...
int ImGui::GetKeyPressedAmount(int key_index, float repeat_delay, float repeat_rate)
{
    ...
    float const t = g.IO.KeysDownDuration[key_index];
...
bool ImGui::IsKeyPressed(int user_key_index, bool repeat)
{
    ...
    if (user_key_index < 0)
        return false;
    IM_ASSERT(user_key_index >= 0 && user_key_index < IM_ARRAYSIZE(g.IO.KeysDown));
    float const t = g.IO.KeysDownDuration[user_key_index];
    if (t == 0.0f)
        return true;
...
bool ImGui::IsMouseClicked(ImGuiMouseButton button, bool repeat)
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(button >= 0 && button < IM_ARRAYSIZE(g.IO.MouseDown));
    float const t = g.IO.MouseDownDuration[button];
    if (t == 0.0f)
        return true;
...
bool ImGui::IsMousePosValid(const ImVec2* mouse_pos)
{
    ...
    float const MOUSE_INVALID = -256000.0f;
...
bool ImGui::BeginChildEx(char const* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    ...
    int const auto_fit_axises = ((size.x == 0.0f) ? (1 << ImGuiAxis_X) : 0x00) | ((size.y == 0.0f) ? (1 << ImGuiAxis_Y) : 0x00);
    if (size.x <= 0.0f)
    ...
    float const backup_border_size = g.Style.ChildBorderSize;
    if (!border)
...
static ImVec2 CalcWindowAutoFitSize(ImGuiWindow* window, const ImVec2& size_contents)
{
    ...
    else
    {
        ...
        bool const is_popup = (window->Flags & ImGuiWindowFlags_Popup) != 0;
        bool const is_menu = (window->Flags & ImGuiWindowFlags_ChildMenu) != 0;
        ...
        int const monitor_idx = window->ViewportAllowPlatformMonitorExtend;
...
static bool ImGui::UpdateManualResize(ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_held, int resize_grip_count, ImU32 resize_grip_col[4])
{
    ...
    int const resize_border_count = g.IO.ConfigWindowsResizeFromEdges ? 4 : 0;
    float const grip_draw_size = IM_FLOOR(ImMax(g.FontSize * 1.35f, window->WindowRounding + 1.0f + g.FontSize * 0.2f));
    float const grip_hover_inner_size = IM_FLOOR(grip_draw_size * 0.75f);
    float const grip_hover_outer_size = g.IO.ConfigWindowsResizeFromEdges ? WINDOWS_RESIZE_FROM_EDGES_HALF_THICKNESS : 0.0f;
    ...
    if (g.NavWindowingTarget && g.NavWindowingTarget->RootWindow == window)
    {
        ...
        {
            float const NAV_RESIZE_SPEED = 600.0f;
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, bool handle_borders_and_resize_grips, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    float const window_rounding = window->WindowRounding;
    float const window_border_size = window->WindowBorderSize;
    if (window->Collapsed)
    {
        // Title bar only
...
void ImGui::RenderWindowTitleBarContents(ImGuiWindow* window, const ImRect& title_bar_rect, char const* name, bool* p_open)
{
    ImGuiContext& g = *GImGui;
    ImGuiStyle& style = g.Style;
    ImGuiWindowFlags flags = window->Flags;

    bool const has_close_button = (p_open != NULL);
    bool const has_collapse_button = !(flags & ImGuiWindowFlags_NoCollapse) && (style.WindowMenuButtonPosition != ImGuiDir_None);
    ...
    char const* UNSAVED_DOCUMENT_MARKER = "*";
    float const marker_size_x = (flags & ImGuiWindowFlags_UnsavedDocument) ? CalcTextSize(UNSAVED_DOCUMENT_MARKER, NULL, false).x : 0.0f;
...
bool ImGui::Begin(char const* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    bool const window_just_created = (window == NULL);
    ...
    int const current_frame = g.FrameCount;
    bool const first_begin_of_the_frame = (window->LastFrameActive != current_frame);
    window->IsFallbackWindow = (g.CurrentWindowStack.Size == 0 && g.WithinFrameScopeWithImplicitWindow);

    // Update the Appearing flag
    bool window_just_activated_by_user = (window->LastFrameActive < current_frame - 1);   // Not using !WasActive because the implicit "Debug" window would always toggle off->on
    bool const window_just_appearing_after_hidden_for_resize = (window->HiddenFramesCannotSkipItems > 0);
    ...
    {
        // Initialize
        bool const window_is_child_tooltip = (flags & ImGuiWindowFlags_ChildWindow) && (flags & ImGuiWindowFlags_Tooltip); // FIXME-WIP: Undocumented behavior of Child+Tooltip for pinned tooltip (#1345)
        ...
        // Apply minimum/maximum window size constraints and size
        window->SizeFull = CalcWindowSizeAfterConstraint(window, window->SizeFull);
        window->Size = window->Collapsed && !(flags & ImGuiWindowFlags_ChildWindow) ? window->TitleBarRect().GetSize() : window->SizeFull;

        // Decoration size
        float const decoration_up_height = window->TitleBarHeight() + window->MenuBarHeight();
        ...
        bool const window_pos_with_pivot = (window->SetWindowPosVal.x != FLT_MAX && window->HiddenFramesCannotSkipItems == 0);
        ...
        if (window->ViewportOwned)
        {
            ...
            bool const is_short_lived_floating_window = (flags & (ImGuiWindowFlags_ChildMenu | ImGuiWindowFlags_Tooltip | ImGuiWindowFlags_Popup)) != 0;
        ...
        bool const handle_borders_and_resize_grips = (window->DockNodeAsHost || !window->DockIsActive);

        // Handle manual resize: Resize Grips, Borders, Gamepad
        int border_held = -1;
        ImU32 resize_grip_col[4] = {};
        int const resize_grip_count = g.IO.ConfigWindowsResizeFromEdges ? 2 : 1; // Allow resize from lower-left if we have the mouse cursor feedback for it.
        float const resize_grip_draw_size = IM_FLOOR(ImMax(g.FontSize * 1.35f, window->WindowRounding + 1.0f + g.FontSize * 0.2f));
        ...
        bool const dim_bg_for_modal = (flags & ImGuiWindowFlags_Modal) && window == GetTopMostPopupModal() && window->HiddenFramesCannotSkipItems <= 0;
        bool const dim_bg_for_window_list = g.NavWindowingTargetAnim && ((window == g.NavWindowingTargetAnim->RootWindow) || (g.NavWindowingList && (window == g.NavWindowingList) && g.NavWindowingList->Viewport != g.NavWindowingTargetAnim->Viewport));
        if (dim_bg_for_modal || dim_bg_for_window_list)
        {
            const ImU32 dim_bg_col = GetColorU32(dim_bg_for_modal ? ImGuiCol_ModalWindowDimBg : ImGuiCol_NavWindowingDimBg, g.DimBgRatio);
        ...
        bool const is_undocked_or_docked_visible = !window->DockIsActive || window->DockTabIsVisible;
        ...
        if (is_undocked_or_docked_visible)
        {
            ...
            bool const title_bar_is_highlight = want_focus || (window_to_highlight && (window->RootWindowForTitleBarHighlight == window_to_highlight->RootWindowForTitleBarHighlight || (window->DockNode && window->DockNode == window_to_highlight->DockNode)));
        ...
        bool const allow_scrollbar_x = !(flags & ImGuiWindowFlags_NoScrollbar) && (flags & ImGuiWindowFlags_HorizontalScrollbar);
        bool const allow_scrollbar_y = !(flags & ImGuiWindowFlags_NoScrollbar);
        float const work_rect_size_x = (window->ContentSizeExplicit.x != 0.0f ? window->ContentSizeExplicit.x : ImMax(allow_scrollbar_x ? window->ContentSize.x : 0.0f, window->Size.x - window->WindowPadding.x * 2.0f - window->ScrollbarSizes.x));
        float const work_rect_size_y = (window->ContentSizeExplicit.y != 0.0f ? window->ContentSizeExplicit.y : ImMax(allow_scrollbar_y ? window->ContentSize.y : 0.0f, window->Size.y - window->WindowPadding.y * 2.0f - decoration_up_height - window->ScrollbarSizes.y));
...
void ImGui::PushMultiItemsWidths(int components, float w_full)
{
    ...
    ImGuiWindow* window = g.CurrentWindow;
    const ImGuiStyle& style = g.Style;
    float const w_item_one  = ImMax(1.0f, IM_FLOOR((w_full - (style.ItemInnerSpacing.x) * (components-1)) / (float)components));
    float const w_item_last = ImMax(1.0f, IM_FLOOR(w_full - (w_item_one + style.ItemInnerSpacing.x) * (components-1)));
...
void ImGui::EndGroup()
{
    ...
    bool const group_contains_curr_active_id = (group_data.BackupActiveIdIsAlive != g.ActiveId) && (g.ActiveIdIsAlive == g.ActiveId) && g.ActiveId;
    bool const group_contains_prev_active_id = !group_data.BackupActiveIdPreviousFrameIsAlive && g.ActiveIdPreviousFrameIsAlive;
...
bool ImGui::BeginPopupModal(char const* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    bool const is_open = Begin(name, p_open, flags);
...
bool ImGui::OpenPopupOnItemClick(char const* str_id, ImGuiMouseButton mouse_button)
{
    ImGuiWindow* window = GImGui->CurrentWindow;
...
bool ImGui::BeginPopupContextItem(char const* str_id, ImGuiMouseButton mouse_button)
{
    ImGuiWindow* window = GImGui->CurrentWindow;
    if (window->SkipItems)
        return false;
...
bool ImGui::BeginPopupContextWindow(char const* str_id, ImGuiMouseButton mouse_button, bool also_over_items)
{
    if (!str_id)
...
bool ImGui::BeginPopupContextVoid(char const* str_id, ImGuiMouseButton mouse_button)
{
    if (!str_id)
        str_id = "void_context";
    ImGuiID id = GImGui->CurrentWindow->GetID(str_id);
...
static void ImGui::NavProcessItem(ImGuiWindow* window, const ImRect& nav_bb, const ImGuiID id)
{
    ...
    if ((g.NavId != id || (g.NavMoveRequestFlags & ImGuiNavMoveFlags_AllowCurrentNavId)) && !(item_flags & (ImGuiItemFlags_Disabled|ImGuiItemFlags_NoNav)))
    {
        ...
        float const VISIBLE_RATIO = 0.70f;
...
float ImGui::GetNavInputAmount(ImGuiNavInput n, ImGuiInputReadMode mode)
{
    ...
    float const t = g.IO.NavInputsDownDuration[n];
...
static void ImGui::NavUpdate()
{
    ...
    if (g.NavWindow && !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) && !g.NavWindowingTarget)
    {
        ...
        float const scroll_speed = IM_ROUND(window->CalcFontSize() * 100 * g.IO.DeltaTime); // We need round the scrolling speed because sub-pixel scroll isn't reliably supported.
...
static float ImGui::NavUpdatePageUpPageDown()
{
    ...
    ImGuiWindow* window = g.NavWindow;
    bool const page_up_held = IsKeyDown(g.IO.KeyMap[ImGuiKey_PageUp]) && !IsActiveIdUsingKey(ImGuiKey_PageUp);
    bool const page_down_held = IsKeyDown(g.IO.KeyMap[ImGuiKey_PageDown]) && !IsActiveIdUsingKey(ImGuiKey_PageDown);
    bool const home_pressed = IsKeyPressed(g.IO.KeyMap[ImGuiKey_Home]) && !IsActiveIdUsingKey(ImGuiKey_Home);
    bool const end_pressed = IsKeyPressed(g.IO.KeyMap[ImGuiKey_End]) && !IsActiveIdUsingKey(ImGuiKey_End);
    if (page_up_held != page_down_held || home_pressed != end_pressed) // If either (not both) are pressed
    {
        ...
        else
        {
            ...
            float const page_offset_y = ImMax(0.0f, window->InnerRect.GetHeight() - window->CalcFontSize() * 1.0f + nav_rect_rel.GetHeight());
            float nav_scoring_rect_offset_y = 0.0f;
            if (IsKeyPressed(g.IO.KeyMap[ImGuiKey_PageUp], true))
            {
...
static void NavUpdateWindowingHighlightWindow(int focus_change_dir)
{
    ...
    int const i_current = ImGui::FindWindowFocusIndex(g.NavWindowingTarget);
...
static void ImGui::NavUpdateWindowing()
{
    ...
    if (g.NavWindowingTarget && g.NavInputSource == ImGuiInputSource_NavGamepad)
    {
        ...
        int const focus_change_dir = (int)IsNavInputTest(ImGuiNavInput_FocusPrev, ImGuiInputReadMode_RepeatSlow) - (int)IsNavInputTest(ImGuiNavInput_FocusNext, ImGuiInputReadMode_RepeatSlow);
    ...
    if (g.NavWindowingTarget && !(g.NavWindowingTarget->Flags & ImGuiWindowFlags_NoMove))
    {
        ...
        {
            float const NAV_MOVE_SPEED = 800.0f;
            float const move_speed = ImFloor(NAV_MOVE_SPEED * g.IO.DeltaTime * ImMin(g.IO.DisplayFramebufferScale.x, g.IO.DisplayFramebufferScale.y)); // FIXME: Doesn't code variable framerate very well
    ...
    if (apply_toggle_layer && g.NavWindow)
    {
        ...
        bool const preserve_layer_1_nav_id = (new_nav_window->DockNodeAsHost != NULL);
...
const ImGuiPayload* ImGui::AcceptDragDropPayload(char const* type, ImGuiDragDropFlags flags)
{
    ...
    bool const was_accepted_previously = (g.DragDropAcceptIdPrev == g.DragDropTargetId);
...
void ImGui::LogRenderedText(const ImVec2* ref_pos, char const* text, char const* text_end)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    bool const log_new_line = ref_pos && (ref_pos->y > g.LogLinePosY + 1);
    if (ref_pos)
    ...
    int const tree_depth = (window->DC.TreeDepth - g.LogDepthRef);
    for (;;)
    {
        // Split the string. Each new line (after a '\n') is followed by spacing corresponding to the current depth of our log entry.
        // We don't add a trailing \n to allow a subsequent item on the same line to be captured.
        char const* line_start = text_remaining;
        char const* line_end = ImStreolRange(line_start, text_end);
        bool const is_first_line = (line_start == text);
        bool const is_last_line = (line_end == text_end);
        if (!is_last_line || (line_start != line_end))
        {
            int const char_count = (int)(line_end - line_start);
...
void ImGui::LogToFile(int auto_open_depth, char const* filename)
{
    ImGuiContext& g = *GImGui;
    if (g.LogEnabled)
        return;
...
void ImGui::LogButtons()
{
    ...
    PushID("LogButtons");
#ifndef IMGUI_DISABLE_TTY_FUNCTIONS
    bool const log_to_tty = Button("Log To TTY"); SameLine();
#else
    bool const log_to_tty = false;
#endif
    bool const log_to_file = Button("Log To File"); SameLine();
    bool const log_to_clipboard = Button("Log To Clipboard"); SameLine();
...
ImGuiWindowSettings* ImGui::CreateNewWindowSettings(char const* name)
{
    ImGuiContext& g = *GImGui;
...
ImGuiWindowSettings* ImGui::FindOrCreateWindowSettings(char const* name)
{
...
static void WindowSettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    for (ImGuiWindowSettings* settings = g.SettingsWindows.begin(); settings != NULL; settings = g.SettingsWindows.next_chunk(settings))
    {
        char const* settings_name = settings->GetName();
...
void ImGui::TranslateWindowsInViewport(ImGuiViewportP* viewport, const ImVec2& old_pos, const ImVec2& new_pos)
{
    ...
    bool const translate_all_windows = (g.ConfigFlagsCurrFrame & ImGuiConfigFlags_ViewportsEnable) != (g.ConfigFlagsLastFrame & ImGuiConfigFlags_ViewportsEnable);
...
static void ImGui::UpdateViewportsNewFrame()
{
    ...
    bool const viewports_enabled = (g.ConfigFlagsCurrFrame & ImGuiConfigFlags_ViewportsEnable) != 0;
    if (viewports_enabled)
    {
        for (int n = 0; n < g.Viewports.Size; n++)
        {
            ImGuiViewportP* viewport = g.Viewports[n];
            bool const platform_funcs_available = viewport->PlatformWindowCreated;
    ...
    bool const is_mouse_dragging_with_an_expected_destination = g.DragDropActive;
...
static void ImGui::UpdateSelectWindowViewport(ImGuiWindow* window)
{
    ...
    if (!lock_viewport)
    {
        ...
        else if (window->Viewport && window != window->Viewport->Window && window->Viewport->Window && !(flags & ImGuiWindowFlags_ChildWindow))
        {
            ...
            bool const will_be_visible = (window->DockIsActive && !window->DockTabIsVisible) ? false : true;
...
void ImGui::UpdatePlatformWindows()
{
    ...
    for (int i = 1; i < g.Viewports.Size; i++)
    {
        ...
        {
            char const* title_begin = window_for_title->Name;
...
static int ImGui::FindPlatformMonitorForRect(const ImRect& rect)
{
    ...
    int const monitor_count = g.PlatformIO.Monitors.Size;
    if (monitor_count <= 1)
        return monitor_count - 1;

    // Use a minimum threshold of 1.0f so a zero-sized rect won't false positive, and will still find the correct monitor given its position.
    // This is necessary for tooltips which always resize down to zero at first.
    float const surface_threshold = ImMax(rect.GetWidth() * rect.GetHeight() * 0.5f, 1.0f);
...
namespace ImGui
{
    ...
    static char const*      DockNodeGetHostWindowTitle(ImGuiDockNode* node, char* buf, int buf_size) { ImFormatString(buf, buf_size, "##DockNode_%02X", node->ID); return buf; }
    ...
    static void*            DockSettingsHandler_ReadOpen(ImGuiContext*, ImGuiSettingsHandler*, char const* name);
    static void             DockSettingsHandler_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, char const* line);
...
static void ImGui::DockContextRemoveNode(ImGuiContext* ctx, ImGuiDockNode* node, bool merge_sibling_into_parent_node)
{
    ...
    bool const merge = (merge_sibling_into_parent_node && parent_node != NULL);
    if (merge)
    {
...
void ImGui::DockContextProcessDock(ImGuiContext* ctx, ImGuiDockRequest* req)
{
    ...
    if (split_dir != ImGuiDir_None)
    {
        ...
        int const split_inheritor_child_idx = (split_dir == ImGuiDir_Left || split_dir == ImGuiDir_Up) ? 1 : 0; // Current contents will be moved to the opposite side
        float const split_ratio = req->DockSplitRatio;
...
static void ImGui::DockNodeRemoveWindow(ImGuiDockNode* node, ImGuiWindow* window, ImGuiID save_dock_id)
{
    ...
    if (node->TabBar)
    {
        ...
        int const tab_count_threshold_for_tab_bar = node->IsCentralNode() ? 1 : 2;
...
static void ImGui::DockNodeUpdate(ImGuiDockNode* node)
{
    ...
    bool const render_dockspace_bg = node->IsRootNode() && host_window && (node_flags & ImGuiDockNodeFlags_PassthruCentralNode) != 0;
    ...
    bool const central_node_hole = node->IsRootNode() && host_window && (node_flags & ImGuiDockNodeFlags_PassthruCentralNode) != 0 && central_node != NULL && central_node->IsEmpty();
...
static void ImGui::DockNodeUpdateTabBar(ImGuiDockNode* node, ImGuiWindow* host_window)
{
    ...
    bool const node_was_active = (node->LastFrameActive + 1 == g.FrameCount);
    bool const closed_all = node->WantCloseAll && node_was_active;
    ...
    const ImGuiDockNodeFlags node_flags = node->GetMergedFlags();
    bool const has_window_menu_button = (node_flags & ImGuiDockNodeFlags_NoWindowMenuButton) == 0;
    bool const has_close_button = (node_flags & ImGuiDockNodeFlags_NoCloseButton) == 0;
   ...
   if (has_window_menu_button)
	...
	int const tabs_count_old = tab_bar->Tabs.Size;
    for (int window_n = 0; window_n < node->Windows.Size; window_n++)
    {
        ImGuiWindow* window = node->Windows[window_n];
    ...
    if (has_close_button && node->VisibleWindow)
    {
        ...
        float const button_sz = g.FontSize;
...
static bool ImGui::DockNodeIsDropAllowed(ImGuiWindow* host_window, ImGuiWindow* root_payload)
{
    ...
    int const payload_count = root_payload->DockNodeAsHost ? root_payload->DockNodeAsHost->Windows.Size : 1;
...
{
    ImGuiContext& g = *GImGui;
    float const dock_spacing = g.Style.ItemInnerSpacing.x;
    const ImGuiAxis axis = (dir == ImGuiDir_Left || dir == ImGuiDir_Right) ? ImGuiAxis_X : ImGuiAxis_Y;
    pos_new[axis ^ 1] = pos_old[axis ^ 1];
    size_new[axis ^ 1] = size_old[axis ^ 1];

    // Distribute size on given axis (with a desired size or equally)
    float const w_avail = size_old[axis] - dock_spacing;
...
bool ImGui::DockNodeCalcDropRectsAndTestMousePos(const ImRect& parent, ImGuiDir dir, ImRect& out_r, bool outer_docking, ImVec2* test_mouse_pos)
{
    ...
    float const parent_smaller_axis = ImMin(parent.GetWidth(), parent.GetHeight());
    float const hs_for_central_nodes = ImMin(g.FontSize * 1.5f, ImMax(g.FontSize * 0.5f, parent_smaller_axis / 8.0f));
...
static void ImGui::DockNodePreviewDockRender(ImGuiWindow* host_window, ImGuiDockNode* host_node, ImGuiWindow* root_payload, const ImGuiDockPreviewData* data)
{
    ...
    bool const is_transparent_payload = g.IO.ConfigDockingTransparentPayload;
    ...
    bool const can_preview_tabs = (root_payload->DockNodeAsHost == NULL || root_payload->DockNodeAsHost->Windows.Size > 0);
    ...
    if (data->IsDropAllowed && can_preview_tabs && data->SplitDir == ImGuiDir_None && data->IsCenterAvailable)
    {
        ...
        int const payload_count = root_payload->DockNodeAsHost ? root_payload->DockNodeAsHost->TabBar->Tabs.Size : 1;
        for (int payload_n = 0; payload_n < payload_count; payload_n++)
        {
    ...
    // Display drop boxes
    float const overlay_rounding = ImMax(3.0f, g.Style.FrameRounding);
    for (int dir = ImGuiDir_None; dir < ImGuiDir_COUNT; dir++)
    {
...
void ImGui::DockNodeTreeUpdatePosSize(ImGuiDockNode* node, ImVec2 pos, ImVec2 size, bool only_write_to_marked_nodes)
{
    ...
    bool const write_to_node = (only_write_to_marked_nodes == false) || (node->MarkedForPosSizeWrite);
    if (write_to_node)
    {
        node->Pos = pos;
        node->Size = size;
    ...
    {
        float const spacing = IMGUI_DOCK_SPLITTER_SIZE;
        const ImGuiAxis axis = (ImGuiAxis)node->SplitAxis;
        float const size_avail = ImMax(size[axis] - spacing, 0.0f);

        // Size allocation policy
        // 1) The first 0..WindowMinSize[axis]*2 are allocated evenly to both windows.
        ImGuiContext& g = *GImGui;
        float const size_min_each = ImFloor(ImMin(size_avail, g.Style.WindowMinSize[axis] * 2.0f) * 0.5f);
...
void ImGui::DockBuilderDockWindow(char const* window_name, ImGuiID node_id)
{
...
void ImGui::DockBuilderCopyWindowSettings(char const* src_name, char const* dst_name)
{
...
void ImGui::DockBuilderCopyDockSpace(ImGuiID src_dockspace_id, ImGuiID dst_dockspace_id, ImVector<char const*>* in_window_remap_pairs)
{
    ...
    {
        char const* src_window_name = (*in_window_remap_pairs)[remap_window_n];
        char const* dst_window_name = (*in_window_remap_pairs)[remap_window_n + 1];
...
void ImGui::BeginDocked(ImGuiWindow* window, bool* p_open)
{
    ...
    bool const auto_dock_node = GetWindowAlwaysWantOwnTabBar(window);
    if (auto_dock_node)
    {
        if (window->DockId == 0)
        {
...
void ImGui::BeginDockableDragDropTarget(ImGuiWindow* window)
{
    ...
    if (AcceptDragDropPayload(IMGUI_PAYLOAD_TYPE_WINDOW, ImGuiDragDropFlags_AcceptBeforeDelivery | ImGuiDragDropFlags_AcceptNoDrawDefaultRect))
    {
        ...
        bool const is_explicit_target = g.IO.ConfigDockingWithShift || IsMouseHoveringRect(explicit_target_rect.Min, explicit_target_rect.Max);

        // Preview docking request and find out split direction/ratio
        //bool const do_preview = true;     // Ignore testing for payload->IsPreview() which removes one frame of delay, but breaks overlapping drop targets within the same window.
        bool const do_preview = payload->IsPreview() || payload->IsDelivery();
...
static void* ImGui::DockSettingsHandler_ReadOpen(ImGuiContext*, ImGuiSettingsHandler*, char const* name)
{
...
static void ImGui::DockSettingsHandler_ReadLine(ImGuiContext* ctx, ImGuiSettingsHandler*, void*, char const* line)
{
    char c = 0;
    int x = 0, y = 0;
    int r = 0;
...
static void ImGui::DockSettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    for (int node_n = 0; node_n < dc->SettingsNodes.Size; node_n++)
    {
        int const line_start_pos = buf->size(); (void)line_start_pos;
...
static void SetClipboardTextFn_DefaultImpl(void*, char const* text)
{
    if (!::OpenClipboard(NULL))
        return;
    int const wbuf_length = ImTextCountCharsFromUtf8(text, NULL) + 1;
...
static void RenderViewportThumbnail(ImDrawList* draw_list, ImGuiViewportP* viewport, const ImRect& bb)
{
    ...
    for (int i = 0; i != g.Windows.Size; i++)
    {
        ...
        bool const window_is_focused = (g.NavWindow && thumb_window->RootWindowForTitleBarHighlight == g.NavWindow->RootWindowForTitleBarHighlight);
...
static void MetricsHelpMarker(char const* desc)
{
    ImGui::TextDisabled("(?)");
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    char const* wrt_rects_names[WRT_Count] = { "OuterRect", "OuterRectClipped", "InnerRect", "InnerClipRect", "WorkRect", "Content", "ContentRegionRect" };
    ...
    char const* trt_rects_names[TRT_Count] = { "OuterRect", "WorkRect", "HostClipRect", "InnerClipRect", "BackgroundClipRect", "ColumnsRect", "ColumnsClipRect", "ColumnsContentHeadersUsed", "ColumnsContentHeadersDesired", "ColumnsContentRowsFrozen", "ColumnsContentRowsUnfrozen" };
    ...
    struct Funcs
    {
        ...
        static void NodeDrawList(ImGuiWindow* window, ImGuiViewportP* viewport, ImDrawList* draw_list, char const* label)
        {
        ...
        static void NodeDockNode(ImGuiDockNode* node, char const* label)
        {
            ImGuiContext& g = *GImGui;
            bool open;
            if (node->Windows.Size > 0)
        ...
        static void NodeTabBar(ImGuiTabBar* tab_bar)
        {
            ...
            char buf[256];
            char* p = buf;
            char const* buf_end = buf + IM_ARRAYSIZE(buf);
        ...
        static void NodeStorage(ImGuiStorage* storage, char const* label)
        {
    ...
    if (ImGui::TreeNode("Docking"))
    {
        ...
        if (ImGui::TreeNode("Settings"))
        {
            ...
            for (int n = 0; n < dc->SettingsNodes.Size; n++)
            {
                ...
                char const* selected_tab_name = NULL;
...
#pragma warning(default : 6386 26495)